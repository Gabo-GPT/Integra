...
(function () {
  ...
  function bindGestion() {
    ...
    function setupCliFormatter(inputEl, boxesEl) {
      if (!inputEl || !boxesEl) return;
      function render() {
        var txt = (inputEl.value || '').replace(/\r\n/g, '\n').replace(/\r/g, '\n');
        if (!txt.trim()) { boxesEl.innerHTML = ''; return; }
        ...
        boxesEl.innerHTML = blocks.map(function (b) {
    ...
    setupCliFormatter($('gestionRed'), $('gestionRedBoxes'));
    setupCliFormatter($('gestionNodo'), $('gestionNodoBoxes'));
  }

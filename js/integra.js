...
(function () {
  ...
  var MODO_LOCAL = typeof window !== 'undefined' && window.INTEGRA_MODO_LOCAL === true;
  var API_URL = '';
  if (!MODO_LOCAL) {
    API_URL = (typeof window !== 'undefined' && window.INTEGRA_API_URL) || '';
  ...
  function loadScript(src) {
    ...
    if (_loadedScripts[base]) return Promise.resolve();
    return new Promise(function (resolve, reject) {
      var existing = document.querySelector('script[src*="' + base + '"]');
      if (existing) { _loadedScripts[base] = true; return resolve(); }
      var s = document.createElement('script');
      s.src = src;
      s.onload = function () { _loadedScripts[base] = true; resolve(); };
      s.onerror = reject;
      document.body.appendChild(s);
    });
  }

  var STORAGE = MODO_LOCAL ? 'integra_data_local' : 'integra_data';
  ...
  function refreshNavByRole(data) {
    ...
    var adminOnlyBlocks = $('dashboardAdminOnlyBlocks');
    var isQoe = role === 'qoe';
    if (adminBlocks) adminBlocks.style.display = (admin || isQoe) ? '' : 'none';
    if (agentBlock) agentBlock.style.display = (admin || isQoe) ? 'none' : '';
    if (adminOnlyBlocks) adminOnlyBlocks.style.display = isQoe ? 'none' : '';
  ...
  function refreshProductividadAgente() {
    ...
    if (role === 'qoe') {
      ...
      wrap.innerHTML =
        ...
        '<div class="noc-content-with-map">' +
        ...
        '<section class="qoe-zona-principal">' +
          '<div class="qoe-diagnostico-card">' +
            ...
            '</div>' +
            '<div class="qoe-analizar-remoto">' +
              '<h4 class="qoe-analizar-titulo">Diagnóstico automático por SSH</h4>' +
              '<div class="qoe-form-row">' +
                '<label for="qoeMac">MAC del m\u00f3dem</label>' +
                '<input type="text" id="qoeMac" placeholder="ej. 0012.3456.7890 o 0012:3456:7890">' +
              '</div>' +
              '<div class="qoe-form-row">' +
                '<label for="qoeCmtsSelect">CMTS</label>' +
                '<input type="text" id="qoeCmtsSelect" list="qoeCmtsList" placeholder="Buscar CMTS por nombre..." autocomplete="off">' +
                '<datalist id="qoeCmtsList"></datalist>' +
              '</div>' +
              '<div class="qoe-analizar-actions">' +
                '<button type="button" class="portal-btn portal-btn-primary" id="qoeBtnAnalizarRemoto">Analizar</button>' +
                '<span class="qoe-analizar-loading" id="qoeAnalizarLoading" style="display:none">Cargando...</span>' +
              '</div>' +
            '</div>' +
        ...
        '<section class="qoe-noc-seccion">' +
          '<div class="qoe-noc-summary-card" id="qoeNocSummaryCard">' +
            ...
            '</div>' +
            '<div class="qoe-noc-summary-interp-row">' +
              ...
              '</div>' +
              '<div class="qoe-noc-chart-nivel-wrap" id="qoeNocChartNivelWrap">' +
                '<div class="qoe-noc-chart-nivel-header">' +
                  '<h4 class="qoe-noc-chart-nivel-title">Salud vs. Saturaci\u00f3n</h4>' +
                  '<span class="qoe-noc-chart-nivel-badge" id="qoeNocChartNivelBadge">—</span>' +
                '</div>' +
                '<svg class="qoe-noc-chart-nivel" id="qoeNocChartNivel" viewBox="0 0 480 220" preserveAspectRatio="xMidYMid meet"></svg>' +
                '<div class="qoe-noc-chart-nivel-capacidad" id="qoeNocChartNivelCapacidad"></div>' +
                '<div class="qoe-noc-chart-nivel-tooltip" id="qoeNocChartNivelTooltip"></div>' +
              '</div>' +
            '</div>' +
            '<div class="qoe-noc-summary-dx-metricas-row">' +
          ...
          '<div class="qoe-noc-card">' +
            ...
            '</div>' +
            '<div class="qoe-noc-main-layout">' +
              '<div class="qoe-noc-left-column">' +
                '<div class="qoe-noc-paneles">' +
                  '<div class="qoe-noc-panel qoe-noc-panel-rf-full">' +
                    '<h4>RF Levels</h4>' +
                    '<div class="qoe-noc-grid qoe-noc-grid-rf">' +
                      '<div class="qoe-noc-metric noc-metric-rf noc-metric-rf-progress" id="qoeNocTx" data-topo="us" data-min="20" data-max="55" data-optmin="36" data-optmax="50"><span class="qoe-noc-metric-label">TX Up</span><div class="noc-rf-value-wrap"><span class="qoe-noc-metric-val qoe-semaforo-muted">—</span><span class="qoe-noc-metric-unit">dBmV</span><div class="noc-sparkline-wrap"><svg class="noc-sparkline" viewBox="0 0 56 18" preserveAspectRatio="none"><polyline points="0,14 7,11 14,13 21,7 28,9 35,5 42,8 49,6 56,9" class="noc-sparkline-line"/></svg></div></div><div class="noc-rf-progress-bar"><div class="noc-rf-progress-track"><div class="noc-rf-progress-optimal"></div><div class="noc-rf-progress-marker"></div></div></div>' +
                      '<div class="qoe-noc-metric noc-metric-rf noc-metric-rf-progress" id="qoeNocRx" data-topo="ds" data-min="-25" data-max="15" data-optmin="-15" data-optmax="10"><span class="qoe-noc-metric-label">RX Down</span><div class="noc-rf-value-wrap"><span class="qoe-noc-metric-val qoe-semaforo-muted">—</span><span class="qoe-noc-metric-unit">dBmV</span><div class="noc-sparkline-wrap"><svg class="noc-sparkline" viewBox="0 0 56 18" preserveAspectRatio="none"><polyline points="0,9 7,7 14,11 21,5 28,7 35,9 42,6 49,8 56,7" class="noc-sparkline-line"/></svg></div></div><div class="noc-rf-progress-bar"><div class="noc-rf-progress-track"><div class="noc-rf-progress-optimal"></div><div class="noc-rf-progress-marker"></div></div></div>' +
                      '<div class="qoe-noc-metric noc-metric-rf noc-metric-rf-progress" id="qoeNocSnrUp" data-topo="us" data-min="0" data-max="45" data-optmin="32" data-optmax="45"><span class="qoe-noc-metric-label">SNR Up</span><div class="noc-rf-value-wrap"><span class="qoe-noc-metric-val qoe-semaforo-muted">—</span><span class="qoe-noc-metric-unit">dB</span><div class="noc-sparkline-wrap"><svg class="noc-sparkline" viewBox="0 0 56 18" preserveAspectRatio="none"><polyline points="0,11 7,9 14,7 21,5 28,3 35,4 42,2 49,3 56,3" class="noc-sparkline-line"/></svg></div></div><div class="noc-rf-progress-bar"><div class="noc-rf-progress-track"><div class="noc-rf-progress-optimal"></div><div class="noc-rf-progress-marker"></div></div></div>' +
                      '<div class="qoe-noc-metric noc-metric-rf noc-metric-rf-progress" id="qoeNocSnrDown" data-topo="ds" data-min="0" data-max="50" data-optmin="35" data-optmax="50"><span class="qoe-noc-metric-label">SNR Down</span><div class="noc-rf-value-wrap"><span class="qoe-noc-metric-val qoe-semaforo-muted">—</span><span class="qoe-noc-metric-unit">dB</span><div class="noc-sparkline-wrap"><svg class="noc-sparkline" viewBox="0 0 56 18" preserveAspectRatio="none"><polyline points="0,13 7,11 14,9 21,7 28,5 35,6 42,4 49,5 56,5" class="noc-sparkline-line"/></svg></div></div><div class="noc-rf-progress-bar"><div class="noc-rf-progress-track"><div class="noc-rf-progress-optimal"></div><div class="noc-rf-progress-marker"></div></div></div>' +
                    '</div>' +
                  '</div>' +
              '<div class="qoe-noc-panel qoe-noc-panel-estabilidad">' +
                '<h4>Estabilidad RF</h4>' +
                '<div class="qoe-noc-grid qoe-noc-grid-dense">' +
                  '<div class="qoe-noc-metric qoe-noc-metric-compact" id="qoeNocFlaps"><span class="qoe-noc-metric-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg></span><span class="qoe-noc-metric-label">Flaps</span><span class="qoe-noc-metric-val qoe-semaforo-muted">—</span></div>' +
                  '<div class="qoe-noc-metric qoe-noc-metric-compact" id="qoeNocCrc"><span class="qoe-noc-metric-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="M12 18v-6"/><path d="M9 15l3 3 3-3"/></svg></span><span class="qoe-noc-metric-label">CRC / Uncorrectables</span><span class="qoe-noc-metric-val qoe-semaforo-muted">—</span></div>' +
                  '<div class="qoe-noc-metric qoe-noc-metric-compact" id="qoeNocRanging"><span class="qoe-noc-metric-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 8v8M8 12l4-4 4 4"/></svg></span><span class="qoe-noc-metric-label">Ranging retries</span><span class="qoe-noc-metric-val qoe-semaforo-muted">—</span></div>' +
                  '<div class="qoe-noc-metric qoe-noc-metric-compact" id="qoeNocUptime"><span class="qoe-noc-metric-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></span><span class="qoe-noc-metric-label">Uptime</span><span class="qoe-noc-metric-val qoe-semaforo-muted">—</span></div>' +
                '</div>' +
              '</div>' +
            ...
            '</div>' +
          '</div>' +
          '<div class="qoe-noc-right-column">' +
            '<div class="qoe-noc-topologia-sticky">' +
              '<span class="qoe-noc-topo-title">Topología HFC</span>' +
              '<div class="qoe-noc-topologia-wrap" id="nocTopologiaWrap">' +
                '<svg class="noc-map-svg noc-topologia-inline" id="nocMapSvg" viewBox="0 0 200 240" xmlns="http://www.w3.org/2000/svg">' +
                  '<ellipse class="noc-topo-node noc-topo-cmts" id="nocTopoCmts" cx="100" cy="45" rx="42" ry="18"/>' +
                  '<text x="100" y="49" text-anchor="middle" font-size="9" font-weight="600">CMTS</text>' +
                  '<line class="noc-topo-line noc-topo-line-us" id="nocTopoLineUs" x1="75" y1="60" x2="50" y2="110"/>' +
                  '<line class="noc-topo-line noc-topo-line-ds" id="nocTopoLineDs" x1="125" y1="60" x2="150" y2="110"/>' +
                  '<ellipse class="noc-topo-node noc-topo-us" id="nocTopoUs" cx="50" cy="122" rx="32" ry="16"/>' +
                  '<text x="50" y="126" text-anchor="middle" font-size="8">US</text>' +
                  '<ellipse class="noc-topo-node noc-topo-ds" id="nocTopoDs" cx="150" cy="122" rx="32" ry="16"/>' +
                  '<text x="150" y="126" text-anchor="middle" font-size="8">DS</text>' +
                  '<line class="noc-topo-line noc-topo-line-modem" x1="70" y1="135" x2="90" y2="180"/>' +
                  '<line class="noc-topo-line noc-topo-line-modem" x1="130" y1="135" x2="110" y2="180"/>' +
                  '<ellipse class="noc-topo-node noc-topo-modem" id="nocTopoModem" cx="100" cy="200" rx="38" ry="16"/>' +
                  '<text x="100" y="204" text-anchor="middle" font-size="8">CPE / MÓDEM</text>' +
                '</svg>' +
              '</div>' +
            '</div>' +
          '</div>' +
  ...
  function bindNocRegistroIntermitencia() {
    var btn = $('nocBtnGuardarIntermitencia');
    if (!btn) return;
    btn.addEventListener('click', function () {
      var cuenta = ($('nocCuenta') ? ($('nocCuenta').value || '').trim() : '') || '';
      var pqr = ($('nocRadicado') ? ($('nocRadicado').value || '').trim() : '') || '';
      var escaladoQoe = (qs('input[name="escaladoQoe"]:checked') || {}).value || 'si';
      var huboSolucion = (qs('input[name="huboSolucion"]:checked') || {}).value || 'si';
      var llsOt = ($('nocLlsOt') ? ($('nocLlsOt').value || '').trim() : '') || '';
      var marcacion = 'Escalado QoE: ' + (escaladoQoe === 'si' ? 'Sí' : 'No');
      var llamada = (huboSolucion === 'no' && llsOt) ? llsOt : '';
      var idPrincipal = cuenta || pqr;
      if (!idPrincipal) {
        showSaveStatus('Completa # Cuenta o Radicado para guardar.', true);
        return;
      }
      addIntermitenciaRegistro(cuenta || pqr, pqr, marcacion, huboSolucion, '', llamada, '', '');
      flushSave();
      refreshIntermitenciaList();
      refreshTableroMensual();
      var rRefresh = typeof refreshGestionOperacionHfc === 'function' ? refreshGestionOperacionHfc : function () {};
      rRefresh();
      var badge = $('qoeCasosValidadosBadge');
      if (badge) badge.textContent = getIntermitenciaRegistros().length + ' Casos validados';
      showSaveStatus('Caso guardado en Intermitencia.');
      var inpCuenta = $('nocCuenta');
      if (inpCuenta) inpCuenta.value = '';
      var inpRad = $('nocRadicado');
      if (inpRad) inpRad.value = '';
      var inpLls = $('nocLlsOt');
      if (inpLls) inpLls.value = '';
      var siQoe = qs('input[name="escaladoQoe"][value="si"]');
      if (siQoe) siQoe.checked = true;
      var siSol = qs('input[name="huboSolucion"][value="si"]');
      if (siSol) siSol.checked = true;
      var llsWrap = $('nocRegistroLlsOtWrap');
      if (llsWrap) llsWrap.hidden = true;
    });
  }
  ...
  function bindNocMapToggle() {
    var panel = $('nocMapPanel');
    var btn = $('nocMapToggle');
    if (!panel || !btn) return;
    btn.addEventListener('click', function () {
      var collapsed = panel.classList.toggle('noc-map-panel-collapsed');
      btn.textContent = collapsed ? '›' : '‹';
      btn.setAttribute('aria-label', collapsed ? 'Mostrar panel de topología' : 'Ocultar panel de topología');
      btn.title = collapsed ? 'Mostrar panel' : 'Ocultar panel';
    });
  }

  function bindQoEAnalizarRemoto() {
    var btn = $('qoeBtnAnalizarRemoto');
    var macInp = $('qoeMac');
    var cmtsInp = $('qoeCmtsSelect');
    var loadingEl = $('qoeAnalizarLoading');
    var modemTa = $('qoeModemOutput');
    var datalist = $('qoeCmtsList');
    if (!btn || !datalist) return;
    var apiBase = (typeof window.INTEGRA_API_BASE !== 'undefined' && window.INTEGRA_API_BASE) ? window.INTEGRA_API_BASE : '';
    function loadCmtsList() {
      fetch(apiBase + '/api/v1/cmts').then(function (r) { return r.ok ? r.json() : []; }).then(function (list) {
        datalist.innerHTML = '';
    ...
    loadCmtsList();
    btn.addEventListener('click', function () {
      var mac = macInp ? (macInp.value || '').trim() : '';
      var cmtsNombre = cmtsInp ? (cmtsInp.value || '').trim() : '';
      if (!mac) {
        alert('Ingresa la MAC del m\u00f3dem.');
        return;
      }
      if (!cmtsNombre) {
        alert('Selecciona un CMTS de la lista.');
        return;
      }
      if (loadingEl) loadingEl.style.display = 'inline';
      btn.disabled = true;
      fetch(apiBase + '/api/v1/analizar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mac: mac, cmtsNombre: cmtsNombre })
      }).then(function (r) {
        if (!r.ok) return r.json().then(function (e) { throw new Error(e.error || 'Error ' + r.status); }).catch(function () { throw new Error('Error ' + r.status); });
        return r.json();
      }).then(function (data) {
        ...
        if (/failed to fetch|networkerror|network error/i.test(String(msg))) {
          msg = 'No se pudo conectar al servidor. Ejecuta "npm start" y abre http://localhost:3000 en el navegador.';
        }
        alert('Error: ' + msg);
      }).finally(function () {
  ...
  function runQoeAnalisisFromOutput(output) {
    var msgEl = $('qoeAnalisisMsg');
    if (!output) return;
    try {
      if (typeof updateQoeNocAnalyzer === 'function') updateQoeNocAnalyzer(output, '');
      if (msgEl) msgEl.style.display = 'none';
  ...
  function updateQoeNocAnalyzer(modemOutput, upstreamOutput) {
    ...
    function setRfProgress(id, val, color) {
      var el = document.getElementById(id);
      if (!el) return;
      var span = el.querySelector('.qoe-noc-metric-val');
      if (span) { span.textContent = val != null && val !== '' ? val : '—'; span.className = 'qoe-noc-metric-val ' + semaforoClass(color); }
      var track = el.querySelector('.noc-rf-progress-track');
      var optimal = el.querySelector('.noc-rf-progress-optimal');
      var marker = el.querySelector('.noc-rf-progress-marker');
      if (!track || !optimal || !marker) return;
      var min = parseFloat(el.getAttribute('data-min')) || 0;
      var max = parseFloat(el.getAttribute('data-max')) || 100;
      var optMin = parseFloat(el.getAttribute('data-optmin')) || min;
      var optMax = parseFloat(el.getAttribute('data-optmax')) || max;
      var n = parseFloat(val);
      var range = max - min || 1;
      var optLeft = ((optMin - min) / range) * 100;
      var optWidth = ((optMax - optMin) / range) * 100;
      var markerPos = isNaN(n) ? 0 : Math.min(100, Math.max(0, ((n - min) / range) * 100));
      optimal.style.left = optLeft + '%';
      optimal.style.width = optWidth + '%';
      optimal.className = 'noc-rf-progress-optimal ' + (color ? 'noc-optimal-' + color : 'noc-optimal-muted');
      ...
      if (p.esMasivo) {
        ...
        if (v.hallazgos && v.hallazgos.length) {
          ...
          v.hallazgos.forEach(function (h) {
            html += '<div class="qoe-hallazgo-card qoe-hallazgo-' + (h.semaforo || 'muted') + '">' +
              '<span class="qoe-hallazgo-led" aria-hidden="true"></span>' +
              '<div class="qoe-hallazgo-body">' +
                '<span class="qoe-hallazgo-title">' + escapeHtml((h.metrica || '').toUpperCase()) + (h.valor != null && h.valor !== '' ? ' (' + escapeHtml(String(h.valor)) + ')' : '') + '</span>' +
                '<span class="qoe-hallazgo-conclusion">' + escapeHtml(h.frase) + '</span>' +
              '</div></div>';
          });
          html += '</div>';
        }
        html += '<div class="qoe-validacion-resumen">' + escapeHtml(v.resumen || '') + '</div>';
        html += '<div class="qoe-recomendacion-agente"><span class="qoe-recomendacion-title">Recomendación para el agente</span><span class="qoe-recomendacion-text">' + escapeHtml(v.recomendacion || '') + '</span></div>';
        if (p.accionOperativa && p.accionOperativa.length) {
    ...
    if (origenEl && diag.confidenceClassification) {
      ...
      origenEl.innerHTML = '<p class="qoe-noc-origen-empty">Ejecuta análisis para ver clasificación de origen.</p>';
    }
    ...
    if (uncorr != null || (diag.raw && diag.raw.uncorrectables != null)) {
      ...
      updateQoeNocGraficaAfectacion();
      updateQoeNocChartNivel(modemH, carrierUtil != null ? carrierUtil : utilDiag);
    }
    ...
    var snrUp = (diag.snrUp && diag.snrUp.valor != null) ? diag.snrUp.valor : (parsed && parsed.modem && parsed.modem.snr != null) ? parsed.modem.snr : null;
    var impactoCongestion = (utilDiag != null && utilDiag < 70 && snrUp != null && snrUp >= 32)
      ? 'El servicio no presenta degradaci\u00f3n por congesti\u00f3n. Portadora con capacidad disponible. SNR de ' + (typeof snrUp === 'number' ? snrUp.toFixed(1) : snrUp) + ' dB indica excelente calidad de se\u00f1al.'
      : null;
    if (interp) {
      ...
      var impactoBase = impactoCongestion || interp.impactoCliente || '—';
      if (ii) ii.textContent = 'Impacto para el cliente: ' + impactoBase;
    ...
    } else if (impactoCongestion) {
      var ii = $('qoeSummaryInterpImpacto');
      if (ii) ii.textContent = 'Impacto para el cliente: ' + impactoCongestion;
    }
  ...
  function updateNocTopologyState(diag) {
    var lineUs = document.getElementById('nocTopoLineUs');
    var lineDs = document.getElementById('nocTopoLineDs');
    var linesModem = document.querySelectorAll('.noc-topo-line-modem');
    if (!lineUs || !lineDs) return;
    var util = (diag.masivoPanel && diag.masivoPanel.utilization != null) ? diag.masivoPanel.utilization : null;
    var usCritical = (diag.tx && diag.tx.color === 'rojo') || (diag.snrUp && diag.snrUp.color === 'rojo') || (util != null && util >= 85);
    var dsCritical = (diag.rx && diag.rx.color === 'rojo') || (diag.snrDown && diag.snrDown.color === 'rojo');
    var usOk = !usCritical && ((diag.tx && diag.tx.color === 'verde') || (diag.snrUp && diag.snrUp.color === 'verde') || (diag.tx && diag.tx.valor != null) || (diag.snrUp && diag.snrUp.valor != null));
    var dsOk = !dsCritical && ((diag.rx && diag.rx.color === 'verde') || (diag.snrDown && diag.snrDown.color === 'verde') || (diag.rx && diag.rx.valor != null) || (diag.snrDown && diag.snrDown.valor != null));
    var modemOnline = (diag.tx && diag.tx.valor != null) || (diag.rx && diag.rx.valor != null) || (diag.snrUp && diag.snrUp.valor != null) || (diag.snrDown && diag.snrDown.valor != null);
    lineUs.classList.remove('noc-topo-line-ok', 'noc-topo-line-critical', 'noc-topo-line-flow');
    if (usCritical) lineUs.classList.add('noc-topo-line-critical'); else if (usOk) lineUs.classList.add('noc-topo-line-ok');
    if (modemOnline && usOk) lineUs.classList.add('noc-topo-line-flow');
    lineDs.classList.remove('noc-topo-line-ok', 'noc-topo-line-critical', 'noc-topo-line-flow');
    if (dsCritical) lineDs.classList.add('noc-topo-line-critical'); else if (dsOk) lineDs.classList.add('noc-topo-line-ok');
    if (modemOnline && dsOk) lineDs.classList.add('noc-topo-line-flow');
    linesModem.forEach(function (ln) {
      ln.classList.remove('noc-topo-line-ok', 'noc-topo-line-critical', 'noc-topo-line-flow');
      if (usCritical || dsCritical) ln.classList.add('noc-topo-line-critical'); else if (modemOnline) { ln.classList.add('noc-topo-line-ok'); ln.classList.add('noc-topo-line-flow'); }
    });
  }

  function bindNocTopologyHover() {
    var metrics = document.querySelectorAll('.qoe-noc-metric[data-topo]');
    var nodeUs = document.getElementById('nocTopoUs');
    var nodeDs = document.getElementById('nocTopoDs');
    if (!nodeUs || !nodeDs) return;
    metrics.forEach(function (m) {
      m.addEventListener('mouseenter', function () {
        var topo = m.getAttribute('data-topo');
        if (topo === 'us') { nodeUs.classList.add('noc-topo-highlight', 'noc-topo-highlight-us'); nodeDs.classList.remove('noc-topo-highlight', 'noc-topo-highlight-ds'); }
        else if (topo === 'ds') { nodeDs.classList.add('noc-topo-highlight', 'noc-topo-highlight-ds'); nodeUs.classList.remove('noc-topo-highlight', 'noc-topo-highlight-us'); }
      });
      m.addEventListener('mouseleave', function () {
        nodeUs.classList.remove('noc-topo-highlight', 'noc-topo-highlight-us');
        nodeDs.classList.remove('noc-topo-highlight', 'noc-topo-highlight-ds');
      });
    });
  }
  ...
  function clearQoeCmtsDiagnostic() {
    ...
    updateQoeNocGraficaAfectacion();
    updateQoeNocChartNivel(null, null, null);
  }
  ...
  function getChartNivelEstado(util) {
    if (util == null || isNaN(util)) {
      return { estado: 'sin_datos', label: '—', color: 'muted', fill: 'rgba(255,255,255,0.1)', fillRgb: '255,255,255' };
    }
    if (util < 70) {
      return { estado: 'optimo', label: 'PORTADORA ESTABLE', color: 'verde', fill: 'rgba(16,185,129,0.35)', fillRgb: '16,185,129' };
    }
    if (util < 85) {
      return { estado: 'advertencia', label: 'CARGA MODERADA', color: 'amarillo', fill: 'rgba(245,158,11,0.4)', fillRgb: '245,158,11' };
    }
    return { estado: 'critico', label: 'SATURACI\u00d3N CR\u00cdTICA', color: 'rojo', fill: 'rgba(239,68,68,0.45)', fillRgb: '239,68,68' };
  }

  function updateQoeNocChartNivel(modemHealth, utilCurrent, diag) {
    var wrap = document.getElementById('qoeNocChartNivelWrap');
    var svgEl = document.getElementById('qoeNocChartNivel');
    var tooltipEl = document.getElementById('qoeNocChartNivelTooltip');
    var badgeChart = document.getElementById('qoeNocChartNivelBadge');
    if (!wrap || !svgEl) return;
    var history = getNocAfectacionHistory();
    var utilRef = utilCurrent != null ? utilCurrent : (history.length ? history[history.length - 1].util : null);
    var chartEstado = getChartNivelEstado(utilRef);
    var pts = [];
    for (var i = 0; i < history.length; i++) {
      var p = history[i];
      var salud = p.snrUp != null ? Math.min(100, Math.round((p.snrUp / 40) * 100)) : (i === history.length - 1 && modemHealth != null ? modemHealth : null);
      var util = p.util != null ? p.util : (i === history.length - 1 && utilCurrent != null ? utilCurrent : null);
      pts.push({ ts: p.ts, salud: salud, util: util, snrUp: p.snrUp, uncorr: p.uncorr });
    }
    if (pts.length === 0 && (modemHealth != null || utilCurrent != null)) {
      var snrVal = (diag && diag.snrUp && diag.snrUp.valor != null) ? diag.snrUp.valor : null;
      var uncVal = (diag && diag.masivoPanel && diag.masivoPanel.uncorrectablesGlobal != null) ? diag.masivoPanel.uncorrectablesGlobal : (diag && diag.raw && diag.raw.uncorrectables);
      pts.push({ ts: Date.now(), salud: modemHealth, util: utilCurrent, snrUp: snrVal, uncorr: uncVal });
    }
    if (pts.length === 1 && (pts[0].salud != null || pts[0].util != null)) {
      pts.unshift({ ts: pts[0].ts - 300000, salud: pts[0].salud, util: pts[0].util, snrUp: pts[0].snrUp, uncorr: pts[0].uncorr });
    }
    var w = 480, h = 220, pad = { t: 20, r: 44, b: 32, l: 44 };
    var gw = w - pad.l - pad.r, gh = h - pad.t - pad.b;
    var svg = '<rect width="100%" height="100%" fill="rgba(11,14,17,0.4)"/>';
    for (var g = 0; g <= 4; g++) {
    ...
    for (var gx = 0; gx <= 4; gx++) {
      ...
      svg += '<line x1="' + xx + '" y1="' + pad.t + '" x2="' + xx + '" y2="' + (pad.t + gh) + '" stroke="rgba(255,255,255,0.05)" stroke-width="1"/>';
    }
    var y70 = pad.t + gh - (0.7 * gh);
    var y85 = pad.t + gh - (0.85 * gh);
    svg += '<line x1="' + pad.l + '" y1="' + y70 + '" x2="' + (w - pad.r) + '" y2="' + y70 + '" stroke="rgba(16,185,129,0.4)" stroke-dasharray="4 3" stroke-width="1"/>';
    svg += '<line x1="' + pad.l + '" y1="' + y85 + '" x2="' + (w - pad.r) + '" y2="' + y85 + '" stroke="rgba(239,68,68,0.7)" stroke-dasharray="4 3" stroke-width="1.5"/>';
    svg += '<text x="' + (w - pad.r + 2) + '" y="' + (y85 + 3) + '" fill="rgba(239,68,68,0.9)" font-size="8">85%</text>';
    if (pts.length === 0) {
      svg += '<text x="' + (w / 2) + '" y="' + (h / 2) + '" fill="rgba(255,255,255,0.4)" font-size="11" text-anchor="middle">Ejecuta an\u00e1lisis para ver gr\u00e1fico</text>';
    } else if (pts.length >= 2) {
      var areaUtil = [], ptsSalud = [], ptsUtil = [];
      for (var j = 0; j < pts.length; j++) {
        var x = pad.l + (gw * j / (pts.length - 1));
        var utilVal = pts[j].util != null ? pts[j].util : 0;
        var saludVal = pts[j].salud != null ? pts[j].salud : 50;
        var yUtil = pad.t + gh - (utilVal / 100) * gh;
        var ySalud = pad.t + gh - (saludVal / 100) * gh;
        ptsUtil.push(x + ',' + yUtil);
        ptsSalud.push(x + ',' + ySalud);
        areaUtil.push(x + ',' + yUtil);
      }
      areaUtil.push((pad.l + gw) + ',' + (pad.t + gh));
      areaUtil.unshift(pad.l + ',' + (pad.t + gh));
      var areaClass = 'noc-chart-area noc-chart-area-' + chartEstado.estado + (chartEstado.estado === 'critico' ? ' noc-chart-area-glow' : '');
      svg += '<polygon class="' + areaClass + '" fill="' + chartEstado.fill + '" stroke="none" points="' + areaUtil.join(' ') + '"/>';
      svg += '<polyline class="noc-chart-line" fill="none" stroke="url(#nocChartSaludGrad)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" points="' + ptsSalud.join(' ') + '"/>';
      for (var k = 0; k < pts.length; k++) {
        var cx = pad.l + (gw * k / (pts.length - 1));
        var utilK = pts[k].util != null ? pts[k].util : 0;
        var saludK = pts[k].salud != null ? pts[k].salud : 50;
        var cySalud = pad.t + gh - (saludK / 100) * gh;
        var snrTxt = pts[k].snrUp != null ? pts[k].snrUp + ' dB' : '—';
        var uncTxt = pts[k].uncorr != null ? pts[k].uncorr.toLocaleString('es-ES') : '—';
        svg += '<circle cx="' + cx + '" cy="' + cySalud + '" r="4" fill="#fbbf24" stroke="rgba(0,0,0,0.3)" stroke-width="1" data-snr="' + snrTxt + '" data-uncorr="' + uncTxt + '" data-ts="' + pts[k].ts + '" class="noc-chart-nivel-point"/>';
      }
    } else if (pts.length === 1 && (pts[0].salud != null || pts[0].util != null)) {
      var x0 = pad.l + gw / 2;
      var u0 = pts[0].util != null ? pts[0].util : 0;
      var s0 = pts[0].salud != null ? pts[0].salud : 50;
      var yu = pad.t + gh - (u0 / 100) * gh;
      var ys = pad.t + gh - (s0 / 100) * gh;
      var areaClass = 'noc-chart-area noc-chart-area-' + chartEstado.estado + (chartEstado.estado === 'critico' ? ' noc-chart-area-glow' : '');
      svg += '<rect class="' + areaClass + '" x="' + pad.l + '" y="' + yu + '" width="' + gw + '" height="' + (pad.t + gh - yu) + '" fill="' + chartEstado.fill + '"/>';
      svg += '<line x1="' + pad.l + '" y1="' + ys + '" x2="' + (w - pad.r) + '" y2="' + ys + '" stroke="#fbbf24" stroke-width="2"/>';
      svg += '<circle cx="' + x0 + '" cy="' + ys + '" r="4" fill="#fbbf24" stroke="rgba(0,0,0,0.3)" data-snr="' + (pts[0].snrUp != null ? pts[0].snrUp + ' dB' : '—') + '" data-uncorr="' + (pts[0].uncorr != null ? pts[0].uncorr.toLocaleString('es-ES') : '—') + '" class="noc-chart-nivel-point"/>';
    }
    svg = '<defs><linearGradient id="nocChartSaludGrad" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#22c55e"/><stop offset="1" stop-color="#fbbf24"/></linearGradient></defs>' + svg;
    var utilFill = chartEstado.estado === 'optimo' ? 'rgba(16,185,129,0.9)' : chartEstado.estado === 'advertencia' ? 'rgba(245,158,11,0.9)' : 'rgba(239,68,68,0.9)';
    svg += '<text x="' + (pad.l - 4) + '" y="' + (pad.t - 2) + '" fill="rgba(34,197,94,0.9)" font-size="8" text-anchor="end">Salud %</text>';
    svg += '<text x="' + (w - pad.r + 4) + '" y="' + (pad.t - 2) + '" fill="' + utilFill + '" font-size="8" text-anchor="start">Util %</text>';
    wrap.classList.add('noc-chart-transitioning');
    svgEl.innerHTML = svg;
    if (badgeChart) {
      badgeChart.textContent = chartEstado.label;
      badgeChart.className = 'qoe-noc-chart-nivel-badge qoe-noc-chart-badge-' + chartEstado.estado;
    }
    var capEl = document.getElementById('qoeNocChartNivelCapacidad');
    if (capEl) {
      if (utilRef != null) {
        var capacidad = Math.round(100 - utilRef);
        capEl.textContent = 'CAPACIDAD DISPONIBLE: ' + capacidad + '%';
        capEl.style.display = '';
        capEl.className = 'qoe-noc-chart-nivel-capacidad' +
          (capacidad > 30 ? ' qoe-noc-capacidad-verde' : capacidad < 15 ? ' qoe-noc-capacidad-rojo' : ' qoe-noc-capacidad-neutral');
      } else {
        capEl.style.display = 'none';
      }
    }
    var badgeEstado = document.getElementById('qoeNocEstado');
    if (badgeEstado && utilRef != null) {
      var badgeText = chartEstado.estado === 'optimo' ? (diag && diag.globalEstado ? diag.globalEstado.texto : 'Estable') : chartEstado.label;
      badgeEstado.textContent = badgeText;
      badgeEstado.className = 'qoe-noc-badge qoe-noc-badge-' + chartEstado.color;
    }
    setTimeout(function () { wrap.classList.remove('noc-chart-transitioning'); }, 100);
    var points = wrap.querySelectorAll('.noc-chart-nivel-point');
    function showTooltip(ev, el) {
      if (!tooltipEl || !el) return;
      var snr = el.getAttribute('data-snr');
      var unc = el.getAttribute('data-uncorr');
      tooltipEl.innerHTML = 'SNR: ' + (snr || '—') + '<br>Uncorrectables: ' + (unc || '—');
      tooltipEl.style.display = 'block';
      tooltipEl.style.left = (ev.clientX - wrap.getBoundingClientRect().left + 10) + 'px';
      tooltipEl.style.top = (ev.clientY - wrap.getBoundingClientRect().top + 10) + 'px';
    }
    function hideTooltip() { if (tooltipEl) tooltipEl.style.display = 'none'; }
    for (var pi = 0; pi < points.length; pi++) {
      points[pi].addEventListener('mouseenter', function (e) { showTooltip(e, this); });
      points[pi].addEventListener('mousemove', function (e) {
        if (tooltipEl && tooltipEl.style.display === 'block') {
          tooltipEl.style.left = (e.clientX - wrap.getBoundingClientRect().left + 10) + 'px';
          tooltipEl.style.top = (e.clientY - wrap.getBoundingClientRect().top + 10) + 'px';
        }
      });
      points[pi].addEventListener('mouseleave', hideTooltip);
    }
    if (!wrap._chartNivelBound) {
      wrap.addEventListener('mouseleave', hideTooltip);
      wrap._chartNivelBound = true;
    }
  }
